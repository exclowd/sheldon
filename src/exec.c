...
int (*builtin_functions[])(WordList *arg) = {
	change_directory,
	print_current_working_directory,
	echo,
	get_process_info,
	list_files_internal,
	(int (*)(WordList *)) exit_successfully,
	set_env,
	unset_env,
	getenv_internal,
	print_jobs,
...
static int execute_system_command(Word *command, WordList *arg, int flag) {
  pid_t child_pid;
  char **argv;
  int status = 0;
...
int execute_compound_command(CompoundCommand *cc) {
  ...
  int ret = 0;
  int output_fd;
  for (SimpleCommandList *curr = cc->_simple_commands; curr != NULL; curr = curr->_next) {
